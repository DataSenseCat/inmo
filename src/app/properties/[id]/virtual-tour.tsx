  ...
  const handleStartTour = async () => {
    ...
    try {
      const imageDataUris = await Promise.all(
        property.images.map(image => urlToDataUri(image.url))
      );
        ...
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0">
          <DialogHeader className="p-6 pb-2">
            <DialogTitle className="font-headline flex items-center gap-2">
              ...
              Tour Virtual IA: {property.title}
            </DialogTitle>
            <DialogDescription>
                Explorá cada habitación con una descripción generada por IA.
            </DialogDescription>
          </DialogHeader>
          <div className="flex-grow min-h-0 flex flex-col px-6 pb-6">
            {loading ? (
                <div className="flex-grow flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-muted-foreground">Generando tu tour virtual...</p>
                    <p className="text-sm text-muted-foreground/80">Esto puede tomar un momento.</p>
                </div>
            ) : (
